...
export default function PaymentClient({
  ...
  useEffect(() => {
    ...
    setCustName((current) => current || customer.name || user.name || "");
    setCustEmail((current) => current || customer.email || user.email || "");
    setCustPhone((current) => current || customer.phone || user.phone || "");
  }, [customer.email, customer.name, customer.phone, user]);

  const nextPath = useMemo(() => {
    const query = searchParams.toString();
    return `/checkout/payment${query ? `?${query}` : ""}`;
  }, [searchParams]);
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response: RazorpaySuccessResponse) => {
          try {
            const verifyAccessToken = await getAccessToken();
    ...
    return (
      <div className="px-4 md:px-10 lg:px-40 py-24 animate-fade-in">
        <div className="max-w-3xl mx-auto text-center bg-white dark:bg-surface-dark border border-gray-100 dark:border-white/10 rounded-3xl p-12">
          ...
          <p className="text-gray-500 dark:text-gray-300 mb-8">
            Checkout is only available for signed-in users. Your order and cart
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] gap-12 items-start">
        <div className="bg-white dark:bg-surface-dark border border-gray-100 dark:border-white/10 rounded-3xl p-8">
          ...
          <div className="mt-10">
            <div className="flex items-center justify-between gap-4 mb-4">
              <h2 className="text-lg font-black">Customer info</h2>
        ...
        <div className="bg-white dark:bg-surface-dark border border-gray-100 dark:border-white/10 rounded-3xl p-8">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-300 mb-6">
            Complete your purchase securely via Razorpay. Your payment and order
            history are saved to your account.
          </p>
          {error && (
          ...
          <div className="mt-4 text-xs text-gray-400 dark:text-gray-300">
            By continuing you agree to the payment terms and confirmation will
            be sent after success.
          </div>
        </div>
      </div>
    </div>
