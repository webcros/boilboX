    ...
    <div className="animate-fade-in">
      ...
      <section className="py-24 px-4 md:px-10 lg:px-40">
        <div className="flex justify-between items-end mb-16">
          <div>
            <h2 className="text-3xl md:text-5xl font-black mb-2">Menu Highlights</h2>
            <p className="text-gray-500 text-lg">Curated for balance, flavor, and vitality.</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {featuredMeals.length > 0 ? (
            featuredMeals.map((meal) => (
              <div key={meal.id} className="bg-white dark:bg-surface-dark rounded-[2rem] overflow-hidden border border-gray-100 dark:border-white/5 hover:shadow-2xl transition-all group">
                <div className="aspect-[4/3] relative overflow-hidden">
                  ...
                  <div className="absolute top-4 left-4 px-3 py-1 bg-white/90 dark:bg-black/80 backdrop-blur-sm rounded-full text-[10px] font-black uppercase tracking-widest">{meal.category}</div>
                </div>
                <div className="p-8">
                  ...
                  <div className="flex items-center justify-between pt-6 border-t border-gray-100 dark:border-white/10">
                    <div className="flex gap-4 text-xs font-bold text-gray-400">
                      <span className="flex items-center gap-1"><span className="material-symbols-outlined !text-sm">local_fire_department</span> {meal.calories} kcal</span>
                      <span className="flex items-center gap-1"><span className="material-symbols-outlined !text-sm">fitness_center</span> {meal.protein} Prot</span>
                    </div>
        ...
        <div className="relative max-w-7xl mx-auto px-4 text-center">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
            <div>
              ...
              <p className="text-gray-400 text-sm">For every 10 meals sold, we feed a child in need through local partners.</p>
            </div>
            <div>
              <div className="text-6xl md:text-8xl font-black text-primary mb-4">24</div>
              <p className="text-xl font-bold mb-2">Local Farms</p>
              <p className="text-gray-400 text-sm">We source directly from small-scale organic farmers within 50 miles.</p>
            </div>
            <div>
              <div className="text-6xl md:text-8xl font-black text-primary mb-4">0%</div>
              <p className="text-xl font-bold mb-2">Food Waste</p>
              <p className="text-gray-400 text-sm">Unsold ingredients are composted or donated to community kitchens daily.</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {TESTIMONIALS.map((t) => (
            <div key={t.id} className="bg-white dark:bg-surface-dark p-10 rounded-[2rem] shadow-sm relative group">
              ...
              <p className="text-xl font-medium leading-relaxed mb-8 relative z-10 italic">"{t.quote}"</p>
      ...
      <section className="py-12 px-4 md:px-10 lg:px-40 pb-24">
        <div className="bg-primary/10 rounded-[2.5rem] p-12 md:p-20 flex flex-col md:flex-row items-center justify-between gap-12 border border-primary/20">
          <div className="text-center md:text-left max-w-xl">
            <h3 className="text-3xl md:text-5xl font-black mb-4 tracking-tight">Want to partner with us?</h3>
            <p className="text-gray-600 dark:text-gray-300 text-lg">Open a franchise, bring a kiosk to your office, or supply fresh local produce.</p>
          </div>
